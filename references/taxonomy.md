...
- [10. Comment-form slop](#10-comment-form-slop)
...
## 10. Comment-form slop

The classes so far are about what a comment *says*. This class is about the comment as a
*shape* — decorative dressing and process narration that agents add around code. The rules
operationalize what code-rot scanners (notably `ai-slop/trivial-comment`,
`ai-slop/narrative-comment`, and `ai-slop/meta-comment` in
[scanaislop/aislop](https://github.com/scanaislop/aislop)) flag, adapted with the same
false-positive discipline as the rest of this file.

**Decorative separator banners.** Runs of `=`/`-`/`*`/`#` with or without a label:
`// =====================`, `# ---- imports ----`, `/* ── Section 2 ── */`,
`// ===== Phase 2 =====`. The file's structure (headings, blank lines, code layout) already
shows the section; the banner is the authoring session decorating. Delete. A banner that
orients a genuinely long, flat file (thousands of lines, no other structure) may earn its keep
— rare, and the label must name a real contract, not a phase.

**Section-header comments.** "// Step 3: validate", "# Phase 2: parse". This is the
phase-label family from class 1 (dead session citations) and the control-flow narration from
class 6 wearing a banner. Delete, or cite the committed owner if the section has one.

**Process commentary.** Two subclasses that must be told apart:

- **Generation provenance — keep.** "Generated by `scripts/build_notebook.py` — edit the
  owner, not this file" and "auto-generated, do not edit" are real contracts: they tell a
  reader (and an agent) that the file is an artifact and where the truth lives. The
  skill's own generated-files-fixed-at-owner rule depends on them. Never trim these.
- **Authoring-session narration — cut.** "// added by the agent while fixing X", "// written
  during the refactor", "// created with help from Claude". The vantage is the session, not
  the project; class 2 (stack and PR vantage) and class 3 (change narration) apply. Delete the
  session, keep any surviving fact it is stapled to.

**Spared rules** (from aislop's own guardrails, worth making explicit):

- **Length alone is not flagged.** A long comment full of real contracts is fine; a short
  trivial comment is still bloat. Judge content, not size.
- **A TODO that links a tracking issue is spared.** "TODO(owner): … (#1470)" is a live
  pointer; the untracked "TODO: fix this later" is planning residue (class 7).
- **Docstrings with meaningful tags survive.** `@param`/`@return`/`Args:`/`Returns:` that
  carry contracts the signature cannot show are required prose; the flagged form is the
  JSDoc/NumPy preamble that restates the member name and nothing else.
