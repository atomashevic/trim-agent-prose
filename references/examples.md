...
- [Comment-form slop](#comment-form-slop)
- [Keeps](#keeps)
...
**Fixed:** delete the banner and the label. The import block already shows itself.

### Generation provenance — keep

**Keep:** `# Generated by scripts/build_notebook.py — edit the owner, then regenerate.`

This is a contract: it tells a reader (and an agent) that the file is an artifact and where the
truth lives. The skill's own generated-files-fixed-at-owner rule depends on it.

### Authoring-session narration — cut

**Leaked:** `# added by the agent while fixing the seed bug`

**Fixed:** `# seed set before any stochastic operation; unseeded runs are not reproducible`

Restate the surviving fact from the project's vantage, then delete the session around it.

### Spared: TODO with a tracking issue

**Keep:** `TODO(ana): port the ESS weights to the design-aware benchmark (#1470)`

The pointer is live and owned. The untracked `TODO: fix this later` is planning residue.
